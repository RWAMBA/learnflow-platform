# Platform — Phase 9: Lovable MVP Prompt

**Scope:** A complete, paste-ready Lovable prompt that builds the MVP from all approved architectural decisions (Phases 1–8).
**Status:** Draft — pending approval. Final phase of the master prompt's sequence.
**How to use this document:** Everything between `===== BEGIN LOVABLE PROMPT =====` and `===== END LOVABLE PROMPT =====` below is meant to be pasted into Lovable as-is. If Lovable's Supabase integration is connected, have it run the SQL in the Database Schema section as a migration first, so the schema is created exactly as designed across Phases 3–5 rather than re-derived from the feature description alone.
...
Run this exact SQL against the connected Supabase project as a migration, in this order, before building any UI on top of it:
...
1. The prompt instructs Lovable to run the exact, approved Phase 5 SQL as a migration rather than re-deriving its own schema from the feature description, preserving fidelity to eight phases of decisions.
2. The single MVP tenant is seeded as `tenant_type = 'family'`, since Phase 1 named homeschooling families as the primary beachhead — not previously stated explicitly as a seeding choice.
3. Seed data for the Content Spine is instructed to be a small representative slice of CBC (a few grades/subjects/lessons), not a complete curriculum — building full curriculum content is a content-operations task, not part of this architecture build.
4. The Dashboard Shell/widget architecture (Phase 4/7) is carried into the prompt explicitly, so Lovable builds one shared, composed shell rather than six independent hardcoded dashboards.
5. The prompt explicitly states what NOT to build, to prevent Lovable from either under-scoping (missing approved MVP features) or over-scoping (building V2/V3 features like AI or live payments prematurely).

### Assumptions
1. Lovable's Supabase integration can execute a provided SQL migration directly; if the connected project isn't empty, the migration should be reviewed for conflicts before running.
2. "Production-ready" (master prompt requirement) is interpreted as clean, typed, tested-where-practical code following the structure in Code Organization — not as fully hardened for the Phase 8 security/infrastructure posture (rate limiting, Sentry, nonce CSP, etc.), which is real follow-up work for the Cursor phase, not something Lovable's initial generation is expected to fully deliver.
3. Seed data specifics (exact grade/subject/lesson names) are left to Lovable's judgment within the stated constraints, rather than fully scripted here.

### Risks
1. **Prompt length vs. Lovable's practical limits:** this is a long, detailed prompt; if Lovable struggles to act on all of it in one pass, split it into stages (schema first, then auth/roles, then Content Spine, then Assignments/Progress, then Messaging, then Dashboards) rather than abandoning fidelity to any one section.
2. **Generated code will need a real review pass:** Lovable output is a strong starting point, not a substitute for the Phase 8 CI/CD, RLS test suite, and security hardening — those need to be applied to whatever Lovable produces, not assumed already present.
3. **Design token fidelity:** Lovable may not apply shadcn/ui CSS variables exactly as specified on the first attempt; verify `globals.css` against Phase 7 after generation.

### Questions Requiring Approval
1. Confirm seeding the single MVP tenant as `tenant_type = 'family'`.
2. Confirm the recommendation to split this prompt into staged passes if Lovable does not handle it well in one shot.
3. This is the final phase in the master prompt's defined sequence — confirm whether you'd like any follow-on work (e.g., reviewing Lovable's actual output against this document, or preparing a Cursor continuation brief).
